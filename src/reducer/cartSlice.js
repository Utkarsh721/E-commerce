...
const cartSlice = createSlice({
  ...
  reducers: {
    addMerchandiseToCart(state, action) {
      ...
      console.log(merchandise, state.cartmerchandise);
    ...
    orderNow(state, action) {
      ...
      console.log(response, current(state.orderList));
      state.cartalbums = [];
    ...
    builder.addCase(getOrderHistoryData.fulfilled, (state, action) => {
      ...
      if (response === undefined) {
        ...
        for (let key in response) {
          const exisitngKey = state.orderList.find((item) => item.key === key);
          console.log(exisitngKey, "existingKey");
          if (!exisitngKey) {
            console.log(response[key].albums);
            state.orderList.push({
              key: key,
              AlbumPrice: response[key].AlbumPrice,
              MerPrice: response[key].MerPrice,
              albums: response[key].albums,
              date: response[key].date,
              mercandise: response[key].mercandise,
            });
          } else {
            return;
          }
        }
        console.log(current(state.orderList));
      }
